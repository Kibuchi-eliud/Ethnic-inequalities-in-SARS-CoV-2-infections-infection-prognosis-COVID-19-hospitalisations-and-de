...
data_full_d<-read.csv("N:/Mediation analysis/outcomes paper/forest_plots/covid19_ethnic_Prog_posthosp_270123_final_v2.csv",stringsAsFactors = F)
str(data_full_d)
...
print(data_full); names(data_full)
...
#============================
data_full1<- subset(data_full, Time=="Prognosis"&Covid19_outcome=="Hospitalisation and death"); head(data_full1)
names(data_full1)
...
geom_vline(xintercept = 1, color = "red", linetype = "dashed", cex = 1, alpha = 0.5)+
  theme_minimal()+
  theme(plot.title = element_text(hjust = 0.15, size=14,face="bold"),
        ...
        panel.border = element_blank(),
        axis.title.x = element_blank(),
        #axis.line.x.bottom=element_line(size = 0.0, colour = "black", linetype=1),
...
#Hospitalisations
#================
data_hosp<- subset(data_full, Time=="Prognosis"&Covid19_outcome=="Hospitalisation"); head(data_hosp)
names(data_hosp)
...
p5<-ggplot(data_hosp, aes(x=HR,y=eth))+
  ...
  geom_vline(xintercept = 1, color = "red", linetype = "dashed", cex = 1, alpha = 0.5)+
  theme_minimal()+
  theme(plot.title = element_text(hjust = 0.35, size=14,face="bold"),
...
data_death<- subset(data_full, Time=="Prognosis"&Covid19_outcome=="Death"); head(data_death)
names(data_death)
...
data_dthhosp<- subset(data_full,Time=="Posthospitalisation "&Covid19_outcome=="Death"); head(data_dthhosp)
names(data_dthhosp); print(data_dthhosp)


#Observations and events for dthhospections

p4_dthhosp<-ggplot(data_dthhosp)+
  geom_text(aes(y = (eth), x = 1, label = Obs_events,fontface = bold_Obs_events), size = 4) + 
  theme_minimal()+
...
#hazard ratio and error bars for dthhospections
data_dthhosp$HR <- as.numeric(data_dthhosp$HR)

p5_dthhosp <-ggplot(data_dthhosp, aes(x=HR,y=eth))+
  geom_point(shape = 18, size = 5)+
  geom_errorbarh(aes(xmin = lower, xmax = upper), height = 0.25)+
  labs(title="Death")+
  geom_vline(xintercept = 1, color = "red", linetype = "dashed", cex = 1, alpha = 0.5)+
  theme_minimal()+
  theme(plot.title = element_text(hjust = 0.50, size=14,face="bold"),
        plot.margin = unit(c(0,0,0,0),'cm'),
        axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank(),
...
p6_dthhosp <- ggplot(data_dthhosp, aes(y=eth)) +
  geom_text(aes(y = (eth), x = 1, label = HR_CI,fontface = bold_HR_CI), size = 4) + 
  ggtitle("HR[95%CI]")+
  theme_minimal()+
  theme(plot.margin = unit(c(0,0,0,0),'cm'),
...
grid.arrange(p1,p2,p3,p4,p5,p6,p1_death,p2_death,p3_death,p4_dthhosp,p5_dthhosp,p6_dthhosp,layout_matrix = lay2)
p_testf6<-grid.arrange(p1,p2,p3,p4,p5,p6,p1_death,p2_death,p3_death,p4_dthhosp,p5_dthhosp,p6_dthhosp,layout_matrix = lay2)
 

ggsave(file="Figure 2_30Jan23.pdf", p_testf6,width = 14.5,height =16, dpi=1000)
